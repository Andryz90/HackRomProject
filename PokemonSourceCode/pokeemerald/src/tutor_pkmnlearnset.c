 ...
 *     is called (see sMoveTutorMovesListTemplate). That callback will reload the contest
...
static void PrintMessageWithPlaceholders(const u8 *src)
{
    ...
    MoveTutorPrintMessage(gStringVar4);
}
...
static void DoMoveTutorMain(void)
{
    ...
    case MENU_STATE_CHOOSE_SETUP_STATE:
        if (!MoveTutorRunTextPrinters())
        {
            FillWindowPixelBuffer(TUTORWIN_MSG, 0x11);
            if (sMoveTutorMenuSate.showContestInfo == FALSE)
            {
                sMoveTutorStruct->state = MENU_STATE_SETUP_BATTLE_MODE;
            }
    ...
    case MENU_STATE_FADE_FROM_SUMMARY_SCREEN:
        ...
        RemoveScrollArrows();
        CopyWindowToVram(TUTORWIN_MSG, COPYWIN_GFX);
        break;
...
static void HideHeartSpritesAndShowTeachMoveText(bool8 onlyHideSprites)
{
    ...
    if (!onlyHideSprites)
    {
        ...
        FillWindowPixelBuffer(TUTORWIN_MSG, 0x11);
        AddTextPrinterParameterized(TUTORWIN_MSG, FONT_NORMAL, gStringVar4, 0, 1, 0, NULL);
    }
}
...
static void CreateLearnableMovesList(void)
{
    ...
    sMoveTutorStruct->numMenuChoices = GetTutorMoves(&gPlayerParty[sMoveTutorStruct->partyMon], sMoveTutorStruct->movesToLearn);
