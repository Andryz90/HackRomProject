...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        if (ability == ABILITY_SWIFT_SWIM       && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_RAIN)
            ...
            speed *= 2;
        else if (gBattleMons[battler].species == SPECIES_CHERRIM_SUNSHINE && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA && gBattleWeather & B_WEATHER_SUN)
            speed = (speed * 150) / 100;
    }
...
static void TurnValuesCleanUp(bool8 var0)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        ...
        gSpecialStatuses[i].formationstate = FORMATION_OFF;
    }
...
void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
{
    ...
    else if (gMovesInfo[move].effect == EFFECT_CHANGE_TYPE)
    {
        gBattleStruct->dynamicMoveType = ptype | F_DYNAMIC_TYPE_SET;
    ...
    if (gMovesInfo[move].type == TYPE_NORMAL
             ...
             && gMovesInfo[move].effect != EFFECT_CHANGE_TYPE
             && gMovesInfo[move].effect != EFFECT_NATURAL_GIFT
             && ((attackerAbility == ABILITY_PIXILATE && (ateType = TYPE_FAIRY))
                 || (attackerAbility == ABILITY_REFRIGERATE && (ateType = TYPE_ICE))
                 || (attackerAbility == ABILITY_AERILATE && (ateType = TYPE_FLYING))
                 || ((attackerAbility == ABILITY_GALVANIZE) && (ateType = TYPE_ELECTRIC))
                 || ((attackerAbility == ABILITY_IGNITE) && (ateType = TYPE_FIRE))
                )
             )
    {
