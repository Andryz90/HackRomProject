...
static void AccuracyCheck(bool32 recalcDragonDarts, const u8 *nextInstr, const u8 *failInstr, u16 move)
{
    ...
    if (move == NO_ACC_CALC_CHECK_LOCK_ON)
    {
        ...
        || (gMovesInfo[move].soundMove && move != MOVE_SING) //Sound moves should skip accuracy check (except sing)
...
static void Cmd_attackanimation(void)
{
    ...
    {
        if (gSpecialStatuses[gBattlerAttacker].parentalBondState == PARENTAL_BOND_2ND_HIT || gSpecialStatuses[gBattlerAttacker].formationstate == FORMATION_IN_PROGRESS) // No animation on second hit
        {
            gBattlescriptCurrInstr = cmd->nextInstr;
            return;
        }
        ...
        if (gBattleControllerExecFlags == 0)
        {
            ...
            else
            {
                ...
                if (IsValidForBattle(&gPlayerParty[*expMonId]))
                {
                    ...
                    if (*B_EXP_CAP_TYPE != EXP_CAP_NONE && gBattleStruct->battlerExpReward != 0)
                    {
...
static void Cmd_jumpifcaptivateaffected(void)
{
    ...
    else if (AreBattlersInfatuable(gBattlerAttacker, gBattlerTarget))
    {
        gBattlescriptCurrInstr = cmd->jumpInstr;
    }
    else
