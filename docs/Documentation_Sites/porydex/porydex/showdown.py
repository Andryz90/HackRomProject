...
def index(moves: dict, species: dict, learnsets: dict, encounters: dict, custom_item_names: list[str] | None = None):
    vanilla_data_dir = pathlib.Path('vanilla')
    abilities = json.load(open(vanilla_data_dir / 'abilities.json', 'r', encoding='utf-8'))
    vanilla_items = json.load(open(vanilla_data_dir / 'items.json', 'r', encoding='utf-8'))
    items = dict(vanilla_items)
    ...
    # Merge custom items parsed from expansion data into the vanilla items table.
    # `custom_item_names` is a list indexed by item-id, containing the display names.
    if custom_item_names:
        # Find next available item number for custom entries (optional, but helps some UIs)
        next_num = max(map(lambda it: it.get('num', 0), items.values()), default=0) + 1
        for name in custom_item_names:
            if not name:
                continue
            # Some parsers may emit placeholders for unused ids
            if name.strip('?') == '':
                continue
    ...
    def alias(id: str, name: str, T: str) -> list[str]:
        index = []
        # Always index the base key (prevents missing entries for single-word names)
        index.append(f'{name_key(name)} {T} {id} 0')
    ...
    def _swap_exact(a: str, b: str) -> None:
        try:
            ia = index.index(a)
            ib = index.index(b)
            index[ia], index[ib] = index[ib], index[ia]
        except ValueError:
            pass

    def _swap_prefix(a_prefix: str, b_prefix: str) -> None:
        ia = next((k for k, s in enumerate(index) if isinstance(s, str) and s.startswith(a_prefix)), None)
        ib = next((k for k, s in enumerate(index) if isinstance(s, str) and s.startswith(b_prefix)), None)
        if ia is not None and ib is not None:
            index[ia], index[ib] = index[ib], index[ia]

        _swap_exact('grass type', 'grass egggroup')
        _swap_exact('fairy type', 'fairy egggroup')
        _swap_exact('flying type', 'flying egggroup')
        _swap_exact('dragon type', 'dragon egggroup')
        _swap_exact('bug type', 'bug egggroup')
        _swap_exact('psychic type', 'psychic egggroup')

        # Ditto: sometimes it appears with extra fields (id/offset), so swap by prefix
        _swap_exact('ditto pokemon', 'ditto egggroup')
        _swap_prefix('ditto pokemon', 'ditto egggroup')
