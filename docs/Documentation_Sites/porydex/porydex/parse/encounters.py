...
def parse_encounter_header(header: InitList,
    ...
    # Gestisce un singolo campo tipo .landMonsInfo / .waterMonsInfo / ecc.
    def handle_field(init: NamedInitializer) -> None:
        if not isinstance(init, NamedInitializer) or not init.name:
            return

        result = None
        field_name = init.name[0].name
    ...
    # 1) Supporta il vecchio formato: campi diretti nello struct
    # 2) Supporta il nuovo formato: campi annidati in .encounterTypes[...]
    for init in field_inits:
        if not isinstance(init, NamedInitializer) or not init.name:
            continue

        # Vecchio formato: campi diretti
        handle_field(init)

        # Nuovo formato: .encounterTypes = { [TIME_*] = { .landMonsInfo = ..., ... }, ... }
        if init.name[0].name == 'encounterTypes':
            container = init.expr
            if isinstance(container, InitList):
                for tod_init in container.exprs:
                    # [TIME_MORNING] = { ... } -> tod_init è un NamedInitializer
                    body = tod_init.expr if isinstance(tod_init, NamedInitializer) else tod_init
                    if not isinstance(body, InitList):
                        continue

                    for inner in body.exprs:
                        handle_field(inner)
