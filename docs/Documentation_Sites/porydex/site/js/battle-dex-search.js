...
var BattlePokemonSearch = /** @class */ (function (_super) {
    ...
    BattlePokemonSearch.prototype.getBaseResults = function () {
        ...
        if (!format)
            {
            if (typeof this.getDefaultResults === 'function') return this.getDefaultResults();
            // Fallback (defensive): build default list from table
            var results = [];
            var table = this.getTable && this.getTable();
            if (table) {
                for (var id in table) {
                    if (!Object.prototype.hasOwnProperty.call(table, id)) continue;
                    results.push([this.searchType || 'pokemon', id]);
                }
            }
            return results;
        }
...
var BattleItemSearch = /** @class */ (function (_super) {
    ...
    BattleItemSearch.prototype.filter = function (row, filters) {
        ...
        if (!item || !item.exists) return true;

        // pokemon filter: show items that are listed as usable by that pokemon (if present)
        if (filters.pokemon) {
            var pkmn = toID(filters.pokemon);
            var users = item.itemUser || item.itemUsers || null;
            if (users && users.length) {
                var ok = false;
                for (var i = 0; i < users.length; i++) {
                    if (toID(users[i]) === pkmn) { ok = true; break; }
                }
                if (!ok) return true;
            }
        }
...
var BattleLocationSearch = /** @class */ (function (_super) {
    ...
    BattleLocationSearch.prototype.getBaseResults = function () { return this.getDefaultResults(); };
