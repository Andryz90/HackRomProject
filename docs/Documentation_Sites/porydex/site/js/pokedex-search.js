// --- Porydex: normalize location display names (remove time suffixes and trailing numbers like " 0") ---
(function () {
	if (typeof window === 'undefined') return;

	window.porydexCleanLocationName = window.porydexCleanLocationName || function (name) {
		if (!name) return name;
		var s = String(name).trim();
	
		// Remove only the time-of-day suffix (do NOT eat numbers before the dash)
		// "Verdanturf Prairie 0 - Morning" -> "Verdanturf Prairie 0"
		// "Route 105 - Morning" -> "Route 105"
		// "Route 0 - Morning" -> "Route 0"   (no longer becomes just "Route")
		s = s.replace(/\s*-\s*(Morning|Day|Night|Evening|Dawn|Dusk)\s*$/i, '');
	
		// Remove trailing ' 0' only for non-Route names (e.g. "Verdanturf Prairie 0")
	...
	window.porydexNormalizeDexLocationsNames = window.porydexNormalizeDexLocationsNames || function () {
		try {
			if (!window.Dex || !Dex.locations) return;

			var dict = Dex.locations.data || Dex.locations.dex || Dex.locations._data || null;
			if (!dict) return;
			...
			for (var id in dict) {
				...
				if (!loc.porydexRawName) loc.porydexRawName = loc.name;

				var cleaned = window.porydexCleanLocationName(loc.porydexRawName);

				// If it collapses to "Route", recover number from id like "route105morning"
				if (cleaned === 'Route') {
					var m = String(id).match(/^route(\d+)/i);
					if (m) cleaned = 'Route ' + m[1];
				}
	...
	window.porydexNormalizeBattleLocationdexNames = window.porydexNormalizeBattleLocationdexNames || function () {
		try {
			...
			for (var zoneid in BattleLocationdex) {
				...
				var z = BattleLocationdex[zoneid];
				if (!z || typeof z.name !== 'string') continue;
			
				// Keep a copy of original name for debugging if needed
				if (!z.porydexRawName) z.porydexRawName = z.name;
			
				// Provide stable ids for link builders that support them
				z.id = zoneid;
				z.zoneid = zoneid;
			
				var cleaned = window.porydexCleanLocationName(z.porydexRawName);
			
				// If it collapses to "Route", recover number from zoneid like "route105morning"
				if (cleaned === 'Route') {
					var m = String(zoneid).match(/^route(\d+)/i);
					if (m) cleaned = 'Route ' + m[1];
				}
...
var PokedexSearchPanel = Panels.Panel.extend({
	...
	initialize: function () {
		if (typeof window !== 'undefined') {
			if (window.porydexNormalizeBattleLocationdexNames) window.porydexNormalizeBattleLocationdexNames();
			if (window.porydexNormalizeDexLocationsNames) window.porydexNormalizeDexLocationsNames();
		}
		...
		if (results.length) {
			...
			if (fragment === 'pokemon/') {
				...
				$searchbox.attr('placeholder', 'Search locations OR filter by pokemon');
				this.$('.buttonbar').remove();
            }
	...
	updateFilters: function() {
		...
		} else if (this.search.qType === 'location') {
