...
function porydexNormalizeLocationdexNames() {
	if (!window.BattleLocationdex) return;
	if (BattleLocationdex.__porydexNamesNormalized) return;
	BattleLocationdex.__porydexNamesNormalized = true;
	...
	for (var zoneid in BattleLocationdex) {
		...
		// per sicurezza: id disponibile a chi genera link
		z.id = zoneid;
		z.zoneid = zoneid;

		// pulisci SOLO il name di display
		z.name = porydexCleanLocationName(z.name);
	}
}
