...
function calculateSMSSSV(gen, attacker, defender, move, field) {
    (0, util_2.checkWindGlider)(attacker, field.attackerSide);
    (0, util_2.checkWindGlider)(defender, field.defenderSide);
...
function calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    if (attacker.hasAbility('Formation') && move.bp > 0 && !move.isMultiHit) {
        var n = (move.hits != null && move.hits > 1)
            ? move.hits
        ...
        if (n > 1) {
            ...
            if (n >= 4)
                W += 0.4 * (n - 3);
            var perHit = W / n;
            var FP = Math.max(1, Math.round(0x1000 * perHit));
            bpMods.push(FP);
            desc.attackerAbility = attacker.ability;
        }
    }
...
function calculateDfModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, hitsPhysical) {
    ...
    else if ((defender.hasItem('Soul Dew') && defender.named('Latios', 'Latias', 'Latios-Mega', 'Latias-Mega') && !hitsPhysical)) {
        dfMods.push(4915);
        desc.defenderItem = defender.item;
    }
